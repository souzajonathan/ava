...
export class GetAllObrasService {
    async execute(query?:filter) {
        ...
        if (query?.obra_nome) {
            where.obra_nome = Raw((alias) => `${alias} ilike '%${query.obra_nome}%'`);
        }
        ...
        if (query?.editora) {
            where.editora = Raw((alias) => `${alias} ilike '%${query.editora}%'`);
        }

        if (query?.periodico_nome) {
            where.periodico_nome = Raw((alias) => `${alias} ilike '%${query.periodico_nome}%'`);
        }

        if (query?.periodico_abreviacao) {
            where.periodico_abreviacao = Raw((alias) => `${alias} ilike '%${query.periodico_abreviacao}%'`);
        }

        const obras = await repo.find({
            relations: ["bibliografias", "obrasAutores", "obrasChildren", "obraParent"],
            where
        });
