...
export class UpdatePpcDisciplinaVersaoController {
    async handle(request: Request, response: Response) {
        ...
        const { ppc_id, disciplina_versao_id, semestre, modulo, competencias_id, perfis_id } = request.body;

        const service = new UpdatePpcDisciplinaVersaoService();

        const result = await service.execute({id, ppc_id, disciplina_versao_id, semestre, modulo, competencias_id, perfis_id});
